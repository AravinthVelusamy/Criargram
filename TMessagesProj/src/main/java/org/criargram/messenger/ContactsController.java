...
public class ContactsController {
    ...
    public void checkAppAccount() {
        ...
        try {
            Account[] accounts = am.getAccountsByType("org.criargram.messenger");
            systemAccount = null;
        ...
        if (UserConfig.getInstance(currentAccount).isClientActivated()) {
            ...
            if (systemAccount == null) {
                try {
                    systemAccount = new Account("" + UserConfig.getInstance(currentAccount).getClientUserId(), "org.criargram.messenger");
    ...
    protected void performSyncPhoneBook(final HashMap<String, Contact> contactHashMap, final boolean request, final boolean first, final boolean schedule, final boolean force, final boolean checkCount, final boolean canceled) {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            /*if (schedule) {
                try {
                    AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                    Account[] accounts = am.getAccountsByType("org.criargram.account");
                    boolean recreateAccount = false;
                    if (UserConfig.getInstance(currentAccount).isClientActivated()) {
                        if (accounts.length != 1) {
                            FileLog.e("detected account deletion!");
                            currentAccount = new Account(UserConfig.getInstance(currentAccount).getCurrentUser().phone, "org.criargram.account");
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.org.criargram.messenger.android.profile");
